...
public class TestNGCustomReportListener implements IReporter {
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outdir) {
        ...
        startHtml(writer);
        writeReportTitle(Constants.REPORT_TITLE);
    ...
    protected PrintWriter createWriter(String outdir) throws IOException {
        ...
        return new PrintWriter(new BufferedWriter(new FileWriter(new File(Constants.REPORT_FILE_NAME))));
    }
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                if (!className.equalsIgnoreCase(lastClassName)) {
                    printTabledata(bufferList, style, lastClassName, mq, cq);
                    mq = 0;
                    buff.setLength(0);
                    bufferList.clear();
                ...
                Calendar excutionTime = Calendar.getInstance();
                excutionTime.setTimeInMillis((end));
                ...
                if (bufferList.size() == 0) {
                    bf.append("<td>" + method.getMethodName() + "</td>").append("<td>" + firstLine + "</td>")
                            .append("<td>" + formatter.format(startCalendar.getTime()) + "</td>")
                            .append("<td>" + formatter.format(excutionTime.getTime()) + "</td>");
                } else {
                    bf.append("<tr class='" + style + (cq % 2 == 0 ? "odd" : "even") + "'><td>" + method.getMethodName()
                            + "</td>").append("<td>" + firstLine + "</td>")
                            .append("<td>" + formatter.format(startCalendar.getTime()) + "</td>")
                            .append("<td>" + formatter.format(excutionTime.getTime()) + "</td></tr>");
                }
                bufferList.add(bf);
    ...
    private void printTabledata(List<StringBuffer> bufferList, String style, String lastClassName, int mq, int cq) {
        for (StringBuffer sBuffer : bufferList) {
            if (bufferList.indexOf(sBuffer) == 0) {
                cq += 1;
                writer.print("<tr class='" + style + (cq % 2 == 0 ? "even" : "odd") + "'>" + "<td");
                if (mq > 1) {
                    writer.print(" rowspan='" + mq + "'");
                }
                writer.println(">" + lastClassName + "</td>");
    ...
    private void startResultSummaryTable(String style) {
        ...
        writer.println("<tr><th>Class</th>"
                + "<th>Method</th><th>Exception Info</th><th>Start Time </th><th>End Time<br/>(hh:mm:ss)</th></tr>");
        m_row = 0;
    }
    ...
    private void summaryCell(String v, boolean isgood) {
        writer.print("<td class='numi" + (isgood ? "''" : "attn") + "'>" + v + "</td>");
    }
